...
layui.define(["jquery", "miniMenu", "element","miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
         ...
         * @param options.clearUrl   后台清理缓存接口 // 因为点击过的左侧导航栏中的模块中子模块地址会被保存到sessionstorage中去
        ...
        render: function (options) {
            ...
            $.getJSON(options.iniUrl, function (data) {
                if (data == null) {
                    ...
                    miniAdmin.renderLogo(data.logoInfo);//渲染主页logo  api/init.json 中配置logo信息
                    miniAdmin.renderClear(options.clearUrl);
                    miniAdmin.renderHome(data.homeInfo); //api/init.json 中主页渲染【renderHome】
                    miniAdmin.renderAnim(options.pageAnim);
                    miniAdmin.listen();
                    miniMenu.render({ //渲染菜单
                        menuList: data.menuInfo,//init.json 中 menuinfo数据  菜单以及菜单目录
                        multiModule: options.multiModule,//开启多模块
                        menuChildOpen: options.menuChildOpen//是否默认展开菜单
                    });
                    miniTab.render({
                        filter: 'layuiminiTab',
        ...
        listen: function () {
            ...
            $('body').on('click', '[data-clear]', function () {
                ...
                sessionStorage.clear(); //点击 页面图标， .layuinimi-clear 
                ...
                if (clearUrl != undefined && clearUrl != '' && clearUrl != null) {
                    $.getJSON(clearUrl, function (data, status) {
                        ...
                        if (data.code != 1) {
                            ...
                            return miniAdmin.success(data.msg); //提示清空网站缓存成功
                        }
                    }).fail(function () {
                        layer.close(loading);
