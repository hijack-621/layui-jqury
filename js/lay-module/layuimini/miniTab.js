...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
        ...
        render: function (options) {
            ...
            miniTab.listenRoll();//监听tab title栏 左右两个箭头图片点击事件【向左向右】
            miniTab.listenSwitch(options); //监听tab切换
            miniTab.listenHash(options);//hash路由
        },
        ...
        create: function (options) {
            ...
            sessionStorage.setItem('layuiminimenu_' + options.tabId, options.title);//添加已点击的页面到session storage中
        },
        ...
        listen: function (options) {
            ...
            $('body').on('mousedown', '.layuimini-tab .layui-tab-title li', function (e) {
                ...
                if (e.which === 3) { //判断是不是鼠标右击
                    miniTab.openTabRignMenu(tabId, left); //鼠标右击
                }
            });
        ...
        listenSwitch: function (options) {
            ...
            element.on('tab(' + options.filter + ')', function (data) { //为 options.filter 这个tab导航 加上tab切换
                var tabId = $(this).attr('lay-id');
        ...
        listenHash: function (options) {
            ...
            if (menu !== undefined) { //是右侧菜单里的tab  创建新tab到tab栏
                miniTab.create({
                    tabId: tabId,
                    href: tabId,
                    title: menu.title,
            ...
            // 判断是否为快捷菜单 layuimini-content-href 为首页快捷入口 的属性 值为 page文件夹中的具体html文件路径
        ...
        listenSwitchMultiModule: function (tabId) {
            $("[layuimini-href]").each(function () {
                if ($(this).attr("layuimini-href") === tabId) {
                    ...
                    // 自动展开菜单栏
                    var addMenuClass = function ($element, type) { //layui-tab-title 栏【tab标签】点击那个  左边导航栏就加上layui-this属性【如果隐藏就展开 去掉layui-nav-itemed】 ，其他原来展开的li就隐藏【加上 layui-nav-itemed】
                        if (type === 1) {
                            $element.addClass('layui-this');
                            if ($element.hasClass('layui-nav-item') && $element.hasClass('layui-this')) {
        ...
        rollPosition: function () {
            ...
            $tabTitle.animate({
                scrollLeft: autoLeft - $tabTitle.width() / 3 //=== 0? //出水平滚动条时，显示元素内容会从这个scrollLeft开始展示
            }, 200);
        },
        ...
        rollClick: function (direction) {
            ...
            if ('left' === direction) {
                $tabTitle.animate({ //点击tabtitle 栏 左右箭头图标，title栏移动450px
                     scrollLeft: left - 450
                }, 200);
            } else {
                $tabTitle.animate({
